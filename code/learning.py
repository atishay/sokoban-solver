...
def perform_action(matrix, action):
  ...
  if matrix.moves < (0 - num_actions_for_fail):
    done = True #Failure
    reward = -1.0
  # print "Moves %d, %s"%(matrix.moves, matrix.actions)
...
while True:
  ...
  observation, reward, done = perform_action(observation, action) #env.step(action)
  reward_sum += reward
  drs.append(reward)
  if done:
    episode_number += 1
    ...
    if episode_number % update_frequency == 0:
      ...
      os.remove('sokoban_checkpoint.h5') if os.path.exists('sokoban_checkpoint.h5') else None
    ...
    prev_x = None
  if reward != 0:
    print ('Episode %d Level %d Result: ' % (episode_number, last_level)) + ('Defeat!' if reward == -1 else 'VICTORY!')
